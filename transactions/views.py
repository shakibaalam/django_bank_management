...
from transactions.constants import DEPOSIT, WITHDRAWAL,LOAN, LOAN_PAID,TRANSFER_IN,TRANSFER_OUT
...
def send_transaction_email2(user,receiver, amount,curr_balance, subject, template):
        message = render_to_string(template, {
            'user' : user,
            'receiver' : receiver,
            'amount' : amount,
...
class TransferMoneyView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            sender_account, recipient_account, amount = form.save()
            ...
            send_transaction_email2(
                user=sender_account.user, 
                receiver=recipient_account.user, 
                amount=amount, 
                curr_balance=sender_account.balance, 
                subject="Send Money", 
                template="transactions/transafer_email.html"
            )
            ...
            send_transaction_email2(
                ...
                amount=amount, 
                curr_balance=recipient_account.balance, 
                subject="Receive Money", 
                template="transactions/recepient_email.html"
            )

            messages.success(request, f'Successfully transferred {amount} to {recipient_account.user.email}')
